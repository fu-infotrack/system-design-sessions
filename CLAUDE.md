...
- **Prefer running the thing over reading about it.** Multiple findings in
  `research/` were corrected by actually standing up Postgres, PgBouncer, or a
...
aspire run                              # Postgres, PgBouncer, Redis. ~60s to ready.
...
Three demos are used in the talk (`03-mutex-scope.sh`, `04-pg-advisory.cs`,
`07-expiry.cs`) and climb the scope ladder — one machine, one database, the
...
- **Ports are pinned high on purpose** (Postgres 55432, PgBouncer 56432, Redis
  56379) so they cannot collide with anything already on a developer's
...
- **Redis is a plain `AddContainer`, not `AddRedis`.** Aspire's Redis
  ...
  stage.
- **PgBouncer runs `POOL_MODE=transaction` with `DEFAULT_POOL_SIZE=1`.** The
  pool size is load-bearing: it forces both clients onto the same backend so
  `04b-pgbouncer-leak.cs` demonstrates the silent mutual-exclusion violation
  rather than a hang.
